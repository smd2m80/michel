...
//before 100ms, after 50ms
class TextReViewController: UIViewController {
    ...
    let xmsLen = 15
    let beforeLen = 10
    let model = try! MAC100_50_prea_yokofk(configuration: MLModelConfiguration())
...
//            textView.resignFirstResponder()
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        attArray = attArray.suffix(beforeLen).map { $0 }
        gyroArray = gyroArray.suffix(beforeLen).map { $0 }
        accArray = accArray.suffix(beforeLen).map { $0 }

        // 処理を現時点から0.1秒後に実行する
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.08) {
            self.attArray = self.attArray.prefix(self.xmsLen).map { $0 }
            self.gyroArray = self.gyroArray.prefix(self.xmsLen).map { $0 }
            self.accArray = self.accArray.prefix(self.xmsLen).map { $0 }
            
            if self.getCoremlOutput()=="cope"{
                self.cFLAG = true
                print("began: cope")
            }else{
                self.cFLAG = false
...
//        print("getCoreOut: ",elapsed)
        ...
        guard let output = try? model.prediction(input:
                                                    MAC100_50_prea_yokofkInput(accX: mlarrayAccX, accY: mlarrayAccY, accZ: mlarrayAccZ, attX: mlarrayAttX, attY: mlarrayAttY, attZ: mlarrayAttZ, gyroX: mlarrayGyroX, gyroY: mlarrayGyroY, gyroZ: mlarrayGyroZ, stateIn: currentState!))
        else {
